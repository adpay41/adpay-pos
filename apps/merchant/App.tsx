...
import { LotteryTab } from './lottery';
...
type Tab = 'sales' | 'tickets' | 'cash' | 'hours' | 'lottery' | 'items' | 'alerts' | 'staff' | 'help';
const TAB_LABEL: Record<Tab, string> = { sales: 'Sales', tickets: 'Tickets', cash: 'Cash', hours: 'Hours', lottery: 'Lottery', items: 'Items', alerts: 'Alerts', staff: 'Staff', help: 'Help' };
...
function Home({ token, onUnauthorized, onSwitch }: { token: string; onUnauthorized: () => void; onSwitch: (t: string) => void }) {
  ...
  const tabs: Tab[] = me
    ? [...(can('reports.view') ? (['sales', 'tickets', 'cash', 'hours', 'lottery'] as const) : []), ...(can('catalog.edit') ? (['items'] as const) : []), ...(can('reports.view') ? (['alerts'] as const) : []), 'staff', ...(flags?.support_chat ? (['help'] as const) : [])]
    : [];
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {current === 'lottery' && <LotteryTab token={token} />}
